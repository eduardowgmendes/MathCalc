...
Quando você tiver uma implementação de um `PageTransformer`, chame `setPageTransformer()` com essa implementação para aplicar as animações personalizadas. É desse modo que obtemos uma animação de deslizar exclusiva e bem útil em nosso painél avançado. Antes de iniciar a ação de deslizar o comportamento padrão do painél avançado é permanecer à direita da tela expondo apenas uma pequena área do painél à mostra. Com isso o usuário intuitivamente pode deslizar o painél para a esquerda para fazer uso das funções matemáticas avançadas que a calculadora possui.  Quando o usuário desliza o painél avançado, o painél numérico padrão fica esmaecido. Isso ocorre porque o método `setAlpha(float alpha)` da classe `View` é invocado quando o método `transformPage(View view, float position)` é chamado a cada ponto de transição da tela, ou seja quanto mais à esquerda o painél avançado estiver, mais o painél numérico padrão ficará esmaecido. Diferentemente do que se pode imaginar, quando o painél numérico principal fica esmaecido, não existe máscara alguma sobre ele. Na verdade o que ocorre é que no segundo construtor da classe `CalculatorPadViewPager` configuramos a cor de fundo como `Color.BLACK`, ou seja, quanto mais o alfa do painél principal é decrementado à medida que deslizamos o painél avançado para a esquerda, mais a cor de fundo aparece.            
```java
  private final PageTransformer mPageTransformer = new PageTransformer() {
        @Override
        public void transformPage(View view, float position) {
